...
def findContours(vision_system, imageParam):
    ...
    if vision_system.message_broker is not None:
...
def all_inside_spray_area(vision_system, contour):
    for pt in contour:
        if cv2.pointPolygonTest(
            vision_system.service.sprayAreaPoints.astype(np.float32),
...
def handle_contour_detection(vision_system,sort=False):
    ...
    if vision_system.service.isCalibrated:
    ...
    for cnt in filtered_contours:
        ...
        if vision_system.service.sprayAreaPoints is None:
